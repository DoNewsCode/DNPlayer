...
- (NSString *)ca_fromTimeStampToDetailDesc {
    ...
    if (createDate.ca_isThisYear) {//是今年
        if (createDate.ca_isToday) {//是今天
            NSDateComponents *components = [createDate ca_deltaWithNow];
        ...
        } else if (createDate.ca_isYesterday) {//z昨天
            dFM.dateFormat = @"昨天 HH:mm";
            return [dFM stringFromDate:createDate];
            
        } else if (createDate.ca_isBeforeYesterday) {//前天
            dFM.dateFormat = @"前天 HH:mm";
            return [dFM stringFromDate:createDate];
        } else if (createDate.ca_isThreeToNineDay) {//三至九天内
            
            return [NSString stringWithFormat:@"%zd天前",createDate.ca_daysBeforToday];
        } else {//十天前
...
- (NSString *)ca_fromTimeStampToFutureDetailDesc {
    ...
    if (createDate.ca_isThisYear) {//是今年
        if (createDate.ca_isToday) {//是今天
            return @"今天";
        } else if (createDate.ca_isTomorrow) {//明天
            return @"明天";
        } else if (createDate.ca_isDayAfterTomorrow) {
            return @"后天";
        } else if (createDate.ca_isThreeToThirtyDay) {
            return [NSString stringWithFormat:@"%zd天后",-createDate.ca_daysBeforToday];
        } else {
            return [NSString stringWithFormat:@"%zd个月后",-createDate.ca_mouthBeforMouth];
        }
    } else {//非今年
